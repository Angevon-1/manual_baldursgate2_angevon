...
class include_irenicus_dungeon(Toggle):
    ...
    When false, it is recommended that you use the Skip Chateau Irenicus mod
    to get out of there easier after starting a new game."""
    display_name = "Include Irenicus's Dungeon"
    default = True

class final_chapter(Choice):
    """Select which chapter to finish the game in. 
    When you choose a later chapter, the previous chapters are automatically enabled. 
...
class include_city_of_caverns(Toggle):
    """When this option is true, key items and locations related to the optional Chapter 4 area, 
    City of Caverns/Sahaguin City will be included in the multiworld.
    When this is on, you'll need to take Saemon's offer to sail away from Spellhold at the end of that questline. 
    If you're not using this option, you can take the portal under Spellhold directly to Chapter 5."""
    display_name = "Include City of Caverns"
    default = False

class gaelan_or_bodhi(Choice):
    """Choose whether you will side with Gaelan Bayle and the Shadow Thieves, 
    or with Bodhi and the vampires, in Chapter 3.
    This affects the names of item and location checks, 
    so be sure to match this with your choice in the game."""
...
class misc_items(Choice):
    """Choose how you'd like miscellaneous items in the item pool. 
    Miscellaneous items are things like the spider figurine and the harp of discord.
    No Misc. Items- No miscellaneous items are in the pool. 
    This works best with the Gibberlings 3 Item Randomizer mod. 
    Base Game Misc. Items Only - Miscellaneous items from the base game are added to the pool. 
    Does not include Collector's Edition or Enhanced Edition items. 
    Base Game Misc. Items and Collector's Edition - Miscellaneous items from the base game are added to the pool, 
    including Collector's Edition items. 
    Base Game Misc. Items and Enhanced Edition - Miscellaneous items from the base game are added to the pool, 
    including Enhanced Edition items.
    Base Game Misc. Items, Collector's Edition, and Enhanced Edition - Miscellaneous items from the base game are added to the pool, 
    including Collector's Edition and Enhanced Edition items.
    """
...
class equipment(Choice):
    """Choose how you'd like equipment in the item pool. Please note the more equipment you add, 
    the more chapters you should include too, else items will be lost during generation.
    No Equipment - No equipment is in the pool. This works best with the Gibberlings 3 Item Randomizer mod. 
    Base Game Magical Equipment Only - Each individual magical equipment item from the base game is added to the pool. 
    Does not include Collector's Edition or Enhanced Edition items. 
    Base Game Magical Equipment and Collector's Edition - Each individual magical equipment item from the base game is added to the pool, 
    including Collector's Edition items. These items might be considered overpowered.
    Base Game Magical Equipment and Enhanced Edition - Each individual magical equipment item from the base game is added to the pool, 
    including Enhanced Edition items.
    Base Game Magical Equipment, Collector's Edition, and Enhanced Edition - Each individual magical equipment item from 
    the base game is added to the pool, including Collector's Edition and Enhanced Edition items.
    """
...
class progressive_equipment(Toggle):
    """Use this option to make equipment progressive. You can't use +x weapons until you've received 
    the same number of progressives for that weapon type. 
    For example, the Namarra short sword is a +2 short sword. 
    You'll need two “Progressive Short Sword” items in order to be able to use it. 
    This option works well with the Gibberlings 3 Item Randomizer mod.
    This CAN be combined with the equipment option above, however items will likely be lost during generation."""
    default = 1
    display_name = "Progressive Equipment"

class include_watchers_keep(Toggle):
    """When this is true, equipment from Watcher's Keep is added to the multiworld. When false, they aren't added.
    Note: Watcher's Keep location checks are NOT affected, they are not implemented at all at this time, 
    this is purely about the item rewards."""
...
class companions(Choice):
    ...
    Companions Only - Each individual non-EE companion is an item. To see what companions are available to you, 
    open the Manual Client and connect to your room. Then open the Manual tab. Then expand the 'Companions' section.
    Companions Only Including Enhanced Edition - as above, but includes the Enhanced Edition companions.
    Quests Only - Adds location checks for the non-EE companion quests. These quests can take a long time to activate in the game; 
    speedrunners should not use any option that includes quests.  
...
class starting_companion_amount(Range):
    """When companions are randomized, use this option to set how many companions you'll 
    already have at the start of the game. They will be added at random to your starting inventory. 
    You'll still have to go and recruit them.
...
class loot_checks(Choice):
    ...
    Containers - Adds every INDIVIDUAL lootable crate/barrel/chest as location checks. To compensate for check count, 
    consumable scrolls will be added to the item pool to add filler. Lockpicking is required and NOT accounted for. 
    Progression may be locked behind a locked chest and it's up to you to get past it."""
...
class forging_checks(Toggle):
    """Set this to true if you'd like to earn a check for forging items like Crom Faeyr, Flail of the Ages, etc. 
    Adds 11 checks and 20 items."""
